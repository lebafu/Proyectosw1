    ...
    public function store(Request $request)
    {
    	...
    	if($id==null)
    	{
    		...
    		//dd($profesor);
    		 ...
    		 {
    			$tesis=DB::table('tesis')->join('comision','tesis.id','=','comision.id')->whereNotnull('tesis.fecha_inscripcion')->where('tesis.profesor_guia','=',$profesor->name)->orwhere('comision.profesor1_comision','=',$profesor->name)->orwhere('comision.profesor2_comision','=',$profesor->name)->orwhere('comision.profesor3_comision','=',$profesor->name)->whereBetween('tesis.fecha_inscripcion',[$fecha_inicio,$fecha_final])->get();
    			dd($tesis);
    		 }
    		 //dd($tesis);
        		$phpWord = new \PhpOffice\PhpWord\PhpWord();
        		$section= $phpWord->addSection();
				$section->addText("TABLA EN WORD",array("size"=>22,"bold"=>true,"align"=>"center")); // Agregamos un titulo al documento con tamaño 22 y en negritas
				...
				foreach($tesis as $tes)
				{
					...
					if($tes->profesor_guia==$profesor->name)
					{
						...
						if($tes->profesor1_comision==$profesor->name or $tes->profesor2_comision==$profesor->name or $tes->profesor3_comision==$profesor_name)
						{
